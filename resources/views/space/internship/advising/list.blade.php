      ...
      <tr>
        ...
        <td class="sub">{{  str_limit($student->internship['intitule'], 100) }}</td>
         ...
         <td class="center">
          ...
          <a class="left" href={{ route('Project.create', ['pfe_id' => $student->internship['id'],'advisor' => '2' ]) }}><i class="tiny material-icons">edit</i></a>
            {{ $student->internship->adviser->adviser2['name']}}
          @else
            <a href={{ route('Project.create', ['pfe_id' => $student->internship['id'],'advisor' => '2' ]) }}><i class="tiny material-icons">add</i></a>
          @endif
          </td>
          <td class="multiline">

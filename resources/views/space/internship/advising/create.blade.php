    ...
    <div class="row">
        <div class="col s12 m6 l6">                        
            <div class="card-panel grey lighten-4">
                @isset($encadrements)
                <h5 class="blue-text"><i class="small material-icons blue-text">subject</i>{{ $encadrements['intitule'] }}</h5>   
                    <h5 class="blue-text lighten-5"><i class="small material-icons blue-text">business</i>
                    {{ $encadrements['raison_sociale'] }}</h6>
                @endisset
            </div>
        </div>
        <div class="col s12 m4 l4">
            <div class="row">
                ...
                {!! Form::open(['action' => ['Internship\AdvisingController@store', 'pfe_id' => $encadrements['id']], 'method' => 'post', 'files' => false]) !!}
                <div class="input-field col s12">
                    <i class="material-icons prefix">supervisor_account</i>
                    {{ Form::select('advisor'.request()->advisor,[null=>'Enseignants ...','NULL'=>'Désactiver']+$profs,null,array('single','id'=>'profs')) }}
                    <input type="hidden" name="pfe_id" value={{ $encadrements['id'] }}>
                    <p class="divider"></p>

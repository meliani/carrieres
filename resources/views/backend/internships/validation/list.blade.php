    ...
    <tbody>
      ...
      <tr>
        ...
        <td><div class="sub strong">{{ $trainee->full_name }}</div>
          @if ($trainee['filiere_text'])
          <span class="new badge blue lighten-3 white-text" 
          data-badge-caption="{{ ( !empty($trainee->filiere_text)? $trainee->filiere_text:'' ) }}">
          </span>
          @endif
        ...
        <td class="sub">{{  isset($trainee->internship->intitule) ? str_limit($trainee->internship->intitule, 100):'' }}</td>
         ...
         <td class="center">
           ...
           {{ __('This agreement was signed') }}{{ __('by') }} {{ $trainee->internship->professor->full_name }}
          @else
            <a class="blue btn-small" href={{ route('backend.projects.create', ['id' => $trainee->internship['id'] ]) }}><i class="tiny material-icons">remove_red_eye</i></a>
          @endif
          </td>
          <td class="center">

    ...
    <table class="responsive-table highlight scale-transition scale-in">
        ...
        <tbody>
            @foreach ($encadrements as $pfe)
                <tr>
                    <td>
                        <div class="">{{ $pfe->student_name }}</div>
                        @if ($pfe->option_text)
                            <span class="new badge blue lighten-3 white-text"
                                data-badge-caption="{{ !empty($pfe->option_text) ? $pfe->option_text : '' }}"></span>
                        @endif
                            ...
                            <ul class="small collection">
                                @foreach ($advisors as $advisor)
                                    @if ($pfe->id == $advisor->id)
                                        <li class="collection-item light-blue-text text-lighten-1">
                                            {{ $advisor->advisorName }}
                                            <span class="new badge blue right lighten-5 blue-text"
                                                data-badge-caption="{{ \Carbon\Carbon::parse($advisor->created_at)->format('d M') }}"></span>
                                        </li>
                                    @endif
                                @endforeach
                            </ul>
                    ...
                    @can('edit advisors')
                        <td class="center">
                            ...
                            <a class="btn-floating btn-small waves-effect waves-light blue @if ($pfe->nbr_advisors >= 2) disabled @endif"
                                href={{ route('pfeEncadrements.create', ['pin' => $pfe->id]) }}><i
